...
(defun ada/replace-tabs-in-buffer ()
...
(defun ada/fix-format-in-buffer ()
...
(defun ada/toggle-comment-on-line ()
  "Comment or uncomment current line"
  (interactive)
  (comment-or-uncomment-region (line-beginning-position) (line-end-position)))

(defun ada/install-package-if-missing (package)
...
(defun ada/set-font-height (h)
  "Interactively change font height attribute"
  (interactive (list (read-number "Enter new height: " (ada/cur-font-height))))
  (set-face-attribute 'default nil :height h))

(defun ada/cur-font-height ()
...
(defun ada/is-windows-p ()
  "Confirm if the running OS is Windows"
  (string= system-type "windows-nt"))

(defun ada/is-macos-p ()
  "Confirm if the running OS is macOS"
  (string= system-type "darwin"))

(defun ada/url-un-escape-region (start end)
...
(defun ada/convert-from-epoch-region (start end)
...
(defun ada/gen-scratch-buffer (&optional switch)
...
(when (ada/is-windows-p) (set-fontset-font t nil "Consolas" nil 'append))
(when (ada/is-macos-p) (set-fontset-font t nil "Monaco" nil 'append))
...
(setq ring-bell-function #'ada/friendlier-visible-bell)
...
(let ((size (if (ada/is-windows-p) 140 160)))
  (ada/set-font-height size))
...
(global-set-key (kbd "C-c f") 'ada/fix-format-in-buffer)
